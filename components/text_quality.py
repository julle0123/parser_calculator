...
class TextQualityChecker(BaseChecker):
    def score(self, elements: list[dict]) -> list[dict]:
        ...
        for el in elements:
            ...
            html_plain = self._strip_html(content.get("html", "")).strip()
            md_plain = self._strip_markdown(content.get("markdown", "")).strip()
            page = el.get("page")
            ...
            garbled_in_el = [c for c in non_space if self._is_garbled(c)]
            if garbled_in_el:
        ...
        if non_space_count >= 20:
            garbled_count = sum(1 for c in all_chars if self._is_garbled(c))
                ...
                consistency_detail["low_similarity_samples"] = [
                    {
                        ...
                        "html": h[:80],
                        "markdown": m[:80],
                        "reason": self._describe_consistency_diff(h, m),
                    }
        ...
        if non_space_count >= 20:
            korean_count = sum(1 for c in all_chars if self._is_korean_syllable(c))
    ...
    def _is_garbled(ch: str) -> bool:
        ...
        code = ord(ch)
        return ch in _GARBLED_CHARS or (_KO_JAMO_START <= code <= _KO_JAMO_END)
